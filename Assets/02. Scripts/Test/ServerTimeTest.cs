...
public class ServerTimeTest : MonoBehaviour
{
    ...
    private void SetAPTo20()
    {
        if (CurrencyManager.Instance != null)
        {
            ...
            GameLog.Log($"<color=green>AP를 {testAPAmount}으로 설정했습니다.</color>");
        }
    }
    ...
    private void CheckAPStatus()
    {
        ...
        GameLog.Log($"<color=cyan>=== AP 상태 ===</color>");
        GameLog.Log($"현재 AP: {currentAP}/{maxAP}");
        GameLog.Log($"다음 회복까지: {remainingTime:F1}초");
    }
    ...
    private void Jump1Hour()
    {
        if (ServerTimeManager.Instance != null)
        {
            ...
            GameLog.Log($"<color=yellow>시간을 {jumpHours}시간 앞으로 점프했습니다! (AP +{jumpHours * 4} 예상)</color>");
            GameLog.Log($"<color=yellow>약 1초 후 AP가 회복됩니다.</color>");
        }
        else
        {
            GameLog.LogWarning("ServerTimeManager가 없습니다!");
        }
    }
    ...
    private void ResetTimeJump()
    {
        if (ServerTimeManager.Instance != null)
        {
            ...
            GameLog.Log("<color=green>시간 오프셋 초기화 완료!</color>");
        }
    }
    ...
    private void CheckServerTime()
    {
        if (ServerTimeManager.Instance != null && ServerTimeManager.Instance.IsSynced)
        {
            ...
            GameLog.Log($"<color=cyan>=== 서버 시간 ===</color>");
            GameLog.Log($"현재 서버 시간: {serverTime:yyyy-MM-dd HH:mm:ss} (UTC)");
            GameLog.Log($"밀리초: {ServerTimeManager.Instance.GetServerTimeMs()}");
        }
        else
        {
            GameLog.LogWarning("ServerTimeManager가 동기화되지 않았습니다!");
        }
    }
    ...
    private void CheckDispatchMode()
    {
        ...
        {
            GameLog.Log("<color=cyan>=== 파견 시스템 상태 ===</color>");
            GameLog.Log("DispatchManager가 활성화되어 있습니다.");
            GameLog.Log("Inspector에서 Use Server Time 확인하세요.");
        }
        else
        {
            GameLog.LogWarning("DispatchManager가 Inspector에서 연결되지 않았습니다!");
        }
    }
    ...
    {
        GameLog.Log("<color=yellow>========== 서버 시간 통합 테스트 시작 ==========</color>");

        CheckServerTime();
        CheckAPStatus();
        CheckDispatchMode();

        GameLog.Log("<color=yellow>========== 테스트 완료 ==========</color>");
        GameLog.Log("<color=green>ContextMenu에서 개별 테스트를 실행하세요:</color>");
        GameLog.Log("1. AP를 20으로 설정");
        GameLog.Log("2. 현재 AP 및 회복 시간 확인");
        GameLog.Log("3. 서버 시간 1시간 점프");
    }
}
